...
### Agent-Native Review Principles
When reviewing code, also verify:
- **Contract alignment:** Do changes maintain consistency with the OpenAPI spec? If the spec changed, did the contract tests update?
- **Real infrastructure tests:** Are there contract tests that hit the real deployed service, or do tests only mock the boundaries?
- **No stub systems:** Are in-memory stores being deployed as if they were production code?
- **CLI coverage:** If a new endpoint was added, is there a CLI command that exercises it?
- **Real-data assertions:** Do tests assert data is real (`assert len(findings) > 0`), not just structurally correct?
